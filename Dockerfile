...
RUN echo "echo 'Welcome to BioLab (v1.1)'" >> $SETUPDIR/init.sh
RUN echo "echo '-----------------'" >> $SETUPDIR/init.sh
RUN echo "echo 'BioLab is a docker container for bioinformatics'" >> $SETUPDIR/init.sh
...
RUN echo "echo 'https://github.com/ahmedmoustafa/BioLab/blob/master/Tools.md'" >> $SETUPDIR/init.sh
RUN echo "echo 'If you use BioLab in your work, please cite: '" >> $SETUPDIR/init.sh
...
RUN mv $SETUPDIR/init.sh /etc/biolab.sh
RUN chmod a+x /etc/biolab.sh

WORKDIR /root/
ENTRYPOINT ["/etc/biolab.sh"]
RUN rm -fr $SETUPDIR
